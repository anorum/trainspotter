...
produced it. Nothing is ever edited - a better detector's backfill adds a
newer layer, and reads resolve "latest ingest wins" per natural key. That is
what makes the time slider retroactively honest when the detector improves.
    ...
    async with pool.acquire() as conn, conn.transaction():
        if observations:
            await conn.executemany(
                ...
                [
                    (
                        o["crossing_id"], o["camera_id"],
                        datetime.fromisoformat(o["captured_at"]),
                        o["detector_version"], o["state"], o["confidence"],
                        o["reason"], o["object_key"],
                    )
