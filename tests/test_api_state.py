...
def frame(minute: float, camera: str = "cam-a", status: str = "ok") -> FrameRecord:
...
def test_feed_blames_odot_when_every_camera_errors() -> None:
    ...
    state = fresh()
    for m in range(3):
        state.apply_frame(frame(float(m), "cam-a", "error"))
        state.apply_frame(frame(float(m), "cam-b", "error"))
...
def test_feed_blames_frozen_cameras_when_polls_succeed_without_new_images() -> None:
    ...
    state = fresh()
    state.apply_frame(frame(0.0, "cam-a", "ok"))
    for m in range(1, 30):
        state.apply_frame(frame(float(m), "cam-a", "not_modified"))
...
def test_feed_blames_us_when_the_poller_goes_silent() -> None:
    ...
    state = fresh()
    state.apply_frame(frame(0.0, "cam-a", "error"))
...
def test_feed_recovers_to_ok_and_flags_the_change() -> None:
    ...
    for m in range(3):
        state.apply_frame(frame(float(m), "cam-a", "error"))
    state.changed = False
    state.apply_frame(frame(3.0, "cam-a", "ok"))
