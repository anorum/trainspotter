...
def test_gold_labels_never_leak_into_training(scene, tmp_path):
    ...
    sessions_file.write_text(
        _session_line(crossing_id, "s-1", session_start, session_end) + "\n"
    )
...
def test_session_core_frames_become_blocked_positives(scene, tmp_path):
    ...
    core_key = _key(
        camera_id, core_ts, f"{int(core_ts.timestamp() * 1000)}-x.jpg"
    )
    edge_key = _key(
        camera_id, edge_ts, f"{int(edge_ts.timestamp() * 1000)}-x.jpg"
    )
...
def test_quiet_period_frames_become_clear_negatives(scene, tmp_path):
    ...
    quiet_key = _key(
        camera_id, quiet_ts, f"{int(quiet_ts.timestamp() * 1000)}-x.jpg"
    )
    near_key = _key(
        camera_id, near_ts, f"{int(near_ts.timestamp() * 1000)}-x.jpg"
    )
...
def test_unclosed_session_still_excludes_frames_from_quiet(scene, tmp_path):
    ...
    sessions_file = sessions_dir / "sessions.jsonl"
    sessions_file.write_text(
        _session_line(crossing_id, "s-open", session_start, None) + "\n"
    )
    gold = tmp_path / "gold.jsonl"
    gold.write_text("")
    ...
    keys = {ex.object_key for ex in examples}
    inside_key = _key(
        camera_id, inside_ts, f"{int(inside_ts.timestamp() * 1000)}-x.jpg"
    )
    latest_key = _key(
        camera_id, latest_ts, f"{int(latest_ts.timestamp() * 1000)}-x.jpg"
    )
