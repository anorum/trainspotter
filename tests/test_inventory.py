...
def test_resolve_marks_the_non_scoring_cameras(inventory):
    """677 and 679 cannot see their crossings (verified by eye 2026-08-12), and
    682's reference verdicts minted phantom sessions; a roster regen must carry
    that policy rather than silently re-enfranchise them. Matching goes through
    the same normalisation as name resolution, so formatting drift cannot drop
    the flag."""
    ...
    for cam_id in ("odot-677", "odot-679", "odot-682"):
        assert by_id[cam_id].get("scores") is False
    for cam_id in ("odot-676", "odot-678", "odot-681"):
