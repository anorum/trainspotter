...
- `detect/` - interchangeable detectors behind one registry (`reference`, `yolo`, `vlm`, the per-camera ONNX `classifier`, and the `auto` router that picks classifier-per-camera and falls back to `reference`).
